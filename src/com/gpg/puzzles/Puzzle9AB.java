...
/**
 * Created by ggillis on 12/13/2017.
 ...
 the stream at the moment because it's full of garbage.
 ...
 characters within it using !: inside garbage, any character that comes after ! should be ignored,
 ...
 The correct answer is 11089 with puzzle9.dat
 ...
 */
public class Puzzle9AB extends PuzzleBase {

    protected boolean DataIsValid(ArrayList<String> inputData) {
        return ValidateSingleLineDataFile(inputData);
    }
    static final Character _groupStart = '{';
    static final Character _groupStop = '}';
    static final Character _garbageStart = '<';
    static final Character _garbageStop = '>';
    static final Character _cancleChar = '!';

    protected int SolveMe(ArrayList<String> inputData) {

        String stream = inputData.get(0);
        int score = 0;
        int groupLevel = 0;
        int garbageCharsRemoved = 0;

        Boolean garbageFound = false;

        System.out.print("Iterating the stream: ");
        for(int i = 0, n = stream.length(); i < n; i++) {
            Character test = stream.charAt(i);
            if (test.equals(_cancleChar)) {
                System.out.print("*!");
                i++;
                continue;
            }
            if (!garbageFound && test.equals(_garbageStart)) {
                System.out.print("*bs");
                garbageFound = true;
                continue;
            }
            ...
            if (garbageFound) {
                ...
                if (test.equals(_groupStart)) {
                    groupLevel++;
                    System.out.print(String.format("*gs%d", groupLevel));
                    continue;
                }
                if (groupLevel > 0 && test.equals(_groupStop)) {
                    System.out.print(String.format("*ge%d", groupLevel));
                    score += groupLevel;
                    groupLevel--;
                    continue;
                }
        ...
        System.out.println(String.format("\nStream processing complete!\nGarbage characters removed = %d", garbageCharsRemoved));
        return score;
    }
}
